...
function displayPosts() {
  ...
  postsToShow.forEach((post, index) => {
    ...
    blogCard.style.animationDelay = `${(index % postsPerPage) * 0.1}s`
...
function openBlogModal(postId) {
  ...
  modalBody.innerHTML = generatePostHTML(post)
...
function generatePostHTML(post) {
  ...
  return `
        ...
        <style>
            .blog-post { font-family: inherit; line-height: 1.6; }
...
function initializeScrollAnimations() {
  ...
  document.querySelectorAll(".stat-item, .mission-item, .blog-card").forEach((el) => {
    el.style.opacity = "0"
